...
const deleteUser = async (userId, password) => {
  try {
    ...
    await Promise.all([
      ...
      //pool.query(`DELETE FROM user_prefrences WHERE userid=${userId}`)
    ]);
...
const updatePreferences = async (userId, preferences) => {
  try {
    ...
    await pool.query("DELETE FROM user_preferences WHERE userid = $1", [
      userId,
    ]);
    ...
    await Promise.all(
      preferences.map(async (catId) => {
        await pool.query(
          "INSERT INTO user_preferences (userid, pref_cat) VALUES ($1, $2)",
          [userId, catId]
        );
      })
    );
